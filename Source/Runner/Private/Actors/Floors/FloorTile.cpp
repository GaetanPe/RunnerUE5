...
#include "../../../Public/Bonus/ScoreMultiplication.h"
#include "../../../Public/Bonus/HealingItem.h"
...
{
	if (IsValid(SmallObstacleClass) && IsValid(BigObstacleClass) && IsValid(CoinClass) && IsValid(MultiplactionClass) && IsValid(HealingClass))
	{
...
void AFloorTile::SpawnLaneItem(UArrowComponent* Lane)
{
	...
	else if (UKismetMathLibrary::InRange_FloatFloat(RandVal, SpawnPercent3,0.7f, true, true))
	{
	...
	{
		AScoreMultiplication* ScoreMult = GetWorld()->SpawnActor<AScoreMultiplication>(MultiplactionClass, SpawnLocation, SpawnParameters);
	}
	else if (UKismetMathLibrary::InRange_FloatFloat(RandVal, 0.05f, 0.1f, true, true))
	{
		AHealingItem* HealingItem = GetWorld()->SpawnActor<AHealingItem>(HealingClass, SpawnLocation, SpawnParameters);
	}
